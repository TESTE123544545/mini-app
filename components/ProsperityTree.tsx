    ...
    >
      <defs>
        ...
        <linearGradient id="pt-wood" x1="0" y1="1" x2="0" y2="0">
          ...
          <stop offset="100%" stopColor="#f2dc8a" />
        </linearGradient>
        ...
        {goldenHalo > 0 && (
          <radialGradient id="pt-golden-halo" cx="50%" cy="35%" r="60%">
            <stop offset="0%" stopColor="#fff6dc" stopOpacity={goldenHalo * 0.55} />
      ...
      <g>
        {flowers.map((flower, index) => (
          <g key={index} opacity={flower.on ? 1 : 0} transform={`scale(${flower.on ? 1 : 0.3})`} style={{ transformOrigin: `${flower.x}px ${flower.y}px`, transition: "opacity .8s ease, transform .8s cubic-bezier(.34,1.56,.64,1)" }}>
            ...
            <circle cx={flower.x} cy={flower.y} r="1.8" fill="#f2dc8a" />
          </g>
        ))}
      </g>
      ...
      {goalProgress !== undefined && goalProgress >= 50 && (
        <circle cx="150" cy="200" r="150" fill="none" stroke="#f7e7ff" strokeOpacity="0.25" strokeWidth="1.4" strokeDasharray="2 6" />
      )}
    </svg>
  );
}
